...
YOUTUBE_API_KEY = 'AIzaSyAm9jHvnxvVUktQMH0vzsN5Qh2IA1dzGcI'


def home(request):
    recent_halls = Hall.objects.all().order_by('-id')[:3]
    popular_halls = Hall.objects.all().order_by('id')[:1]
...
def video_search(request):
    ...
    if search_form.is_valid():
        ...
        #'https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=6&q=eggs&key=[YOUR_API_KEY]'
...
class UpdateHall(LoginRequiredMixin,generic.UpdateView):
    ...
    def get_object(self):
        video = super(UpdateHall, self).get_object()
